...
export default forwardRef<HTMLDivElement, Message>(
  function Message({ message, isNextMessageSamePerson }, ref) {
    return (
      <div
        ref={ref}
        ...
        <div className={cn('relative flex h-6 w-6 aspect-square items-center justify-center', {
          ...
          {message.isUserMessage ? (
            <Icons.user className="fill-zinc-200 text-zinc-200 h3/4 w-3/4" />
          ): (
            <Icons.logo className="fill-zinc-300 h3/4 w-3/4" />
          )}
        </div>
            ...
            }
            {
              message.id !== 'loading-message' ? (
                <div className={cn('text-xs select-none mt-2 w-full text-right', {
                  ...
                  {new Date(message.createdAt).toLocaleDateString('en-SG', {
                    hour: 'numeric',
                    minute: 'numeric',
                    hour12: false
                  })}
